...
def fillObsCPT(bayesNet, gameState):
    ...
    #Helper Funcion to find adjacent quadrant
    ...
    for house in house_locations:
        for obs in gameState.getHouseWalls(house):
            ...
            #iterate through rows of obscpt
            for i in obsCPT:
                ghostHouseVal = i[GHOST_HOUSE_VAR]
                foodHouseVal = i[FOOD_HOUSE_VAR]
                color = i[obsVar]
                quadrant = getAdjQuadrant(gameState, obs)
                p = 0
                ...
                #If the adjacent house center is occupied by the ghost house, it is red with probability PROB_GHOST_RED and blue otherwise.
                    ...
                    if color == RED_OBS_VAL:
                        ...
                        p = 0
                obsVarFactor.setProbability(i, p)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assignment in queryFactor.getAllPossibleAssignmentDicts():
        temp = queryFactor.getProbability(assignment)
        if temp > p:
            p = temp
            best = assignment
    return best
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        query_factor = inference.inferenceByVariableElimination(self.bayesNet, HOUSE_VARS, evidence, eliminationOrder)
        leftp, rightp = 0, 0
        ...
        leftExpectedValue = leftp * WON_GAME_REWARD + (1 - leftp) * GHOST_COLLISION_REWARD
        rightExpectedValue = rightp * WON_GAME_REWARD + (1 - rightp) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        for p, expEvidence in self.getExplorationProbsAndOutcomes(evidence):
            #newEvidence = evidence
            newEvidence = evidence.copy()
            newEvidence.update(expEvidence)
            # you can call computeEnterValues again with the hypothetical evidence provided by getExplorationProbsAndOutcomes.
            left, right = self.computeEnterValues(newEvidence, enterEliminationOrder)
            expVal = max(left, right)
            expectedValue = expectedValue + expVal * p
