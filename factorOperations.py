...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        #get the non eliminated conditional variables
        """calculate the set of unconditioned variables and conditioned 
        variables for the factor obtained by eliminating the variable
        eliminationVariable."""
        uncondTemp = factor.unconditionedVariables()
        uncond = [var for var in uncondTemp if var != eliminationVariable]
        ...
        eliminationVariable are summed with rows that match
        assignments on the other variables."""
        for assignment in returnedFactor.getAllPossibleAssignmentDicts():
            p = 0
            for elim in domain:
                assignmentCopy = assignment.copy()
                assignmentCopy[eliminationVariable] = elim
                p = p + factor.getProbability(assignmentCopy)
            returnedFactor.setProbability(assignment, p)
        return returnedFactor
...
def normalize(factor):
    ...
    "*** YOUR CODE HERE ***"
    #It takes a Factor as input and normalizes it, that is, it scales all of the entries in the Factor such that the
    # sum of the entries in the Factor is 1. If the sum of probabilities in the input factor is 0, you should return None.
    ...
    sum = 0
    old = factor.getAllPossibleAssignmentDicts()
    for row in old:
    ...
    for var in uncond:
        if len(variableDomainsDict[var]) == 1:
            cond.add(var)
    uncond = [var for var in uncond if var not in cond]
    ...
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        ...
        newFactor.setProbability(assignment, prob / sum)
    return newFactor
